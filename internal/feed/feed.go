...
type Repository interface {
	...
	// 개별 게시글 저장에 실패하더라도 나머지 게시글의 처리는 계속 진행되며, 반환값으로 실제로 저장에 성공한 게시글 수를 돌려줍니다.
	InsertArticles(providerID string, articles []*Article) (int, error)

	// GetArticles 지정한 providerID와 boardIDs에 해당하는 게시글을 최신 작성일시 순으로 최대 제한 개수(limit)만큼 반환합니다.
	GetArticles(providerID string, boardIDs []string, limit uint) ([]*Article, error)
	...
	GetLatestCrawledInfo(providerID, boardID string) (string, time.Time, error)

	// UpdateLatestCrawledArticleID 새로운 게시글 수집이 끝난 후, 가장 마지막에 수집한 게시글의 ID를 저장합니다.
	// 다음 크롤링 때 이전에 수집한 글을 중복해서 가져오지 않도록 반드시 호출해야 합니다.
	UpdateLatestCrawledArticleID(providerID, boardID, articleID string) error
}
